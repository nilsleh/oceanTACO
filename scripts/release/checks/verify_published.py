...
for name, (size, kind, reference_count) in REFERENCE_SETS.items():
    ...
    header = __import__("json").loads(
        (directory / "header.json").read_text(encoding="utf-8")
    )
    ...
    expected_positions = build_positions(mask, patch_size=patch_size, kind=kind)
    ...
    for r in qs.coverage:
        ...
        for tok in ("swot", "ssh"):
            ...
            if voc > vc:
                bad["ocean>valid"] += 1
            if vc > p[f"{tok}_footprint_cells"]:
                bad["valid>footprint"] += 1
        if r["argo_profile_count"] is None:
            nulls["argo"] += 1
        if r["swot_valid_cells"] == 0 and r["swot_n_obs_sum"] not in (0, None):
            bad["nobs_no_cells"] += 1
    n = len(qs.coverage)
    st = collections.Counter(a["status"] for a in qs.assets)
    sel = select_queryset(
        qs,
        QueryFilter(coverage=(CoverageRequirement("swot", "valid_ocean_cells", 1.0),)),
    )
    print(
        f"{name:14s} pos={len(expected_positions):6d} dates={D} cov={n:>10,} assets={len(qs.assets):,} "
        f"null%: swot={100 * nulls['swot'] / n:5.2f} ssh={100 * nulls['ssh'] / n:5.2f} argo={100 * nulls['argo'] / n:5.2f} "
        f"violations={sum(bad.values())} {dict(st)} swot>=1:{sel.count:,}",
        flush=True,
    )
    if bad:
        print("   !!! VIOLATIONS", dict(bad), flush=True)
