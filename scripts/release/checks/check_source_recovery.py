...
def _mask_values(
    mask, values: np.ndarray, lat: np.ndarray, lon: np.ndarray
) -> np.ndarray:
    ...
    lat_index -= np.abs(mask.lat[lat_index - 1] - flat_lat) <= np.abs(
        mask.lat[lat_index] - flat_lat
    )
    lon_index -= np.abs(mask.lon[lon_index - 1] - flat_lon) <= np.abs(
        mask.lon[lon_index] - flat_lon
    )
...
def _swot_misses(rows, mask, open_ocean, lats, lons, patch) -> int:
    ...
    for row in rows:
        with xr.open_dataset(row["uri"], engine="h5netcdf") as dataset:
            axes.append(
                (
                    np.asarray(dataset["lat"].values, dtype=float),
                    _canonical_lon(dataset["lon"].values),
                )
            )
    ...
    for row, (lat, lon) in zip(rows, axes, strict=True):
        with xr.open_dataset(row["uri"], engine="h5netcdf") as dataset:
            ...
            lon = lon[order]
            values = np.asarray(dataset[spec.primary_variable].squeeze().values)[
                :, order
            ]
            rs = np.searchsorted(global_lat, lat)
            cs = np.searchsorted(global_lon, lon)
            eligible = _mask_values(
                mask, open_ocean, *np.meshgrid(lat, lon, indexing="ij")
            )
            missed += int(
                (np.isfinite(values) & eligible & ~covered[np.ix_(rs, cs)]).sum()
            )
    return missed
...
def check(root: Path, date_count: int, sets: tuple[str, ...] = SETS) -> list[str]:
    ...
    for name in sets:
        ...
        lons, lats = (
            positions.column("centre_lon").to_numpy(),
            positions.column("centre_lat").to_numpy(),
        )
        groups = defaultdict(list)
        for row in pq.read_table(
            directory / "assets.parquet",
            columns=["date_index", "token", "uri", "status"],
        ).to_pylist():
        ...
        for date_index in _sample_dates(len(header["dates"]), date_count):
            ...
            if swot:
                missed["l3_swot"] += _swot_misses(
                    swot, mask, open_ocean, lats, lons, patch
                )
            for row in groups.get((date_index, "argo"), []):
                with xr.open_dataset(row["uri"], engine="h5netcdf") as dataset:
                    point_lat = np.asarray(dataset["lat"].values, dtype=float).reshape(
                        -1
                    )
                    ...
                    if keep.any():
                        missed["argo"] += int(
                            (
                                ~_contains_points(
                                    point_lat[keep], point_lon[keep], lats, lons, patch
                                )
                            ).sum()
                        )
        print(
            f"{name:14s} sampled-dates={date_count} missed-swot={missed['l3_swot']} missed-argo={missed['argo']}",
            flush=True,
        )
        for token, count in missed.items():
            if count:
                failures.append(
                    f"{name}: {count} open-ocean {token} observations are outside every published footprint"
                )
    return failures
...
def main() -> int:
    ...
    parser.add_argument(
        "--sets", default=",".join(SETS), help="comma-separated released set names"
    )
