...
from ocean_taco.retrieve import _REGION_BOUNDS, _clean_swot, _intersects, _merge_points
from ocean_taco.sampling.coverage import (
...
parser.add_argument(
    "--root", type=Path, default=Path(os.environ.get("ROOT", "release/querysets/v2"))
)
...
for setname in (name for name in args.sets.split(",") if name):
    ...
    for _ in range(args.draws):
        pi = (
            ...
            else random.randrange(len(qs.positions))
        )
        di = (
            args.date_index
            if args.date_index is not None
            else random.randrange(len(qs.dates))
        )
        if not 0 <= pi < len(qs.positions) or not 0 <= di < len(qs.dates):
            raise ValueError(
                "--position-index/--date-index lie outside the selected QuerySet."
            )
        pos = qs.positions[pi]
        lon0, lat0 = float(pos["centre_lon"]), float(pos["centre_lat"])
        date = qs.dates[di][:10]
        dd = date.replace("-", "_")
        ...
        for token, key in (("l3_ssh", "ssh"), ("l3_swot", "swot")):
            ...
            for p in paths:
                ...
                if spec.primary_variable not in d:
                    d.close()
                    usable = None
                    break
                usable.append((p, d))
            if usable is None:
                for _, d in usable or []:
                    d.close()
                if row[f"{key}_valid_cells"] is not None:
                    print(
                        f"EXPECTED NULL {setname} {date} {token} {lon0:.2f},{lat0:.2f} got {row[f'{key}_valid_cells']}"
                    )
                    fail += 1
                else:
                    ok += 1
            ...
            for p, ds in usable:
                ...
                ds = (
                    ds.assign_coords(
                        lon=np.where(
                            lon == 180.0, -180.0, ((lon + 180.0) % 360.0) - 180.0
                        )
                    )
                    .sortby("lon")
                    .sortby("lat")
                )
                keep = [spec.primary_variable] + (
                    ["n_obs"] if token == "l3_swot" else []
                )
                parts.append(ds[keep].load())
                ds.close()
            merged = (
                xr.combine_by_coords(parts, combine_attrs="override")
                if len(parts) > 1
                else parts[0]
            )
            merged[spec.primary_variable].attrs["units"] = spec.canonical_unit
            ref = measure_dense_coverage(
                merged,
                token=token,
                patch_size=ps,
                centre_lon=lon0,
                centre_lat=lat0,
                ocean_mask=mask,
                n_obs_variable="n_obs" if token == "l3_swot" else None,
            )
            got = (row[f"{key}_valid_cells"], row[f"{key}_valid_ocean_cells"])
            exp = (ref.valid_cells, ref.valid_ocean_cells)
            if got != exp:
                print(
                    f"MISMATCH {setname} {date} {token} {lon0:.2f},{lat0:.2f} got={got} exp={exp}"
                )
                fail += 1
            else:
                ok += 1
            if token == "l3_swot":
                if row["swot_n_obs_sum"] != ref.n_obs_sum:
                    print(
                        f"NOBS MISMATCH {setname} {date} {lon0:.2f},{lat0:.2f} got={row['swot_n_obs_sum']} exp={ref.n_obs_sum}"
                    )
                    fail += 1
                else:
                    ok += 1
        ...
        if all(os.path.exists(p) for p in ap):
            ...
            ref = measure_argo_profile_count(
                _merge_points(tiles),
                patch_size=ps,
                centre_lon=lon0,
                centre_lat=lat0,
            ...
            if row["argo_profile_count"] != ref:
                print(
                    f"ARGO MISMATCH {setname} {date} {lon0:.2f},{lat0:.2f} got={row['argo_profile_count']} exp={ref}"
                )
                fail += 1
            else:
                ok += 1
            ...
            if row["argo_profile_count"] is not None:
                print(f"EXPECTED ARGO NULL {setname} {date}")
                fail += 1
            else:
                ok += 1
